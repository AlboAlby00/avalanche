...
class MetaSER(SupervisedTemplate):
    """
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.r = 0.5
        self.old_model = None
        self.long_term_memory = deepcopy(self.model).to(self.device)
    ...
    def training_epoch(self, **kwargs):
        ...
        for self.mbatch in self.dataloader:
            ...
            with torch.no_grad():
                ...
                long_term_memory_output = avalanche_forward(
                    self.long_term_memory, self.mb_x, self.mb_task_id
                )
            ...
            if self.replay_loader is not None:
                ...
                if self.use_long_term_memory:
                    self.loss += self.beta * F.mse_loss(
                        long_term_memory_output[self.batch_size_mem :],
                        self.mb_output[self.batch_size_mem :],
                    )
                else:
                    self.loss += self.beta * F.mse_loss(
            ...
            a = random.random()
            if self.use_long_term_memory and self.r > a:
                with torch.no_grad():  # Disable gradients for manual parameter updates
                    for long_term_memory_param, model_param in zip(
                        self.long_term_memory.parameters(), self.model.parameters()
                    ):
                        long_term_memory_param.data = (
                            self.gamma * model_param.data
                            + (1 - self.gamma) * long_term_memory_param.data
                        )
